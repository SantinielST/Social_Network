    ...
    : Controller
{
    ...
    private async Task<ChatViewModel> GenerateChat(string id)
    {
        ...
        // Берем сообщения
        var repository = unitOfWork.GetRepository<MessageEntity>() as MessageRepository;
    ...
    public async Task<IActionResult> NewMessage(string id, ChatViewModel chat)
    {
        ...
        {
            Sender = mapper.Map<User>(currentUserEntity), // можно оставить для отображения в BLL
            Recipient = mapper.Map<User>(friendEntity),
            Text = chat.NewMessage.Text
        };
        
        var messageEntity = mapper.Map<MessageEntity>(item);

        messageEntity.Sender = currentUserEntity;
        messageEntity.Recipient = friendEntity;
        messageEntity.SenderId = currentUserEntity.Id;
        messageEntity.RecipientId = friendEntity.Id;
